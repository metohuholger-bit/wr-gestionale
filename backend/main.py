...
async def get_sheet_data():
    ...
    for row in reader:
        ...
        # Pulisci Note — rimuovi blocco tecnico TIM
        note = clean.get("Note", "") or ""
        if note and "ID NTW SAP:" in note:
            idx = note.find("ID NTW SAP:")
            # Trova la fine del blocco tecnico (ultimo _ prima del testo utile)
            dopo = note[idx:]
            import re as re2
            match = re2.search(r'_WBS:\s*\S*\s*_?\s*', dopo)
            if match:
                note_pulita = (note[:idx] + dopo[match.end():]).strip()
                clean["Note"] = note_pulita if note_pulita else ""
