...
function MappaSub({ wr, onClose, API, user, onSquadraCreata }) {
  ...
  const creaSquadra = async () => {
    ...
    try {
      ...
      setNomeSquadra('');
      setSelected(new Set());
      Object.values(markersRef.current).forEach(m => m.setStyle({ fillColor: '#3b82f6' }));
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      wr.forEach(w => {
        ...
        const marker = L.circleMarker([lat, lon], { radius: 9, fillColor: '#3b82f6', color: 'white', weight: 2, fillOpacity: 0.9 })
          .addTo(map)
          .bindPopup(`<div style="font-family:monospace;font-size:12px"><b style="color:#3b82f6">WR ${w.WR}</b><br/>${w.Indirizzo||''}, ${w.Localita||''}<br/>Stato: <b>${w.StatoWR}</b></div>`);
      ...
      <div style={{ width: '92vw', height: '88vh', background: 'var(--panel)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {selected.size > 0 && (
          <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', background: 'rgba(245,158,11,0.08)', display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <button onClick={() => { setSelected(new Set()); Object.values(markersRef.current).forEach(m => m.setStyle({ fillColor: '#3b82f6' })); }}
